...
if (!defined('__CLASS_HTML2PDF__'))
{
	...
	require_once(dirname(__FILE__).'/parsingHTML.php');			// classe de parsing HTML
	require_once(dirname(__FILE__).'/styleHTML.php');			// classe de gestion des styles
	...
	class HTML2PDF
	{
		...
		var	$isInForm			= false;	// indique si on est dans un formulaire. Contient dans ce cas l� l'action de celui-ci
		...
		var	$defaultFont		= null;		// fonte par d�faut si la fonte demand�e n'existe pas
		...
		*/
		...
		/**
		* renseigner l'encoding � utiliser
		*
...
*/
		...
		*/
		function writeHTML($html, $vue = false)
		{
            ...
            // Biswajit:0009582: Implement currency veriable in Qutation & invoice template.
...
//			if (($w==0 && $this->style->value['width']==0) || ($w>$this->style->value['width']) || $this->style->value['position']=='absolute')
		...
		*/	
		function o_BARCODE($param)
		{
			...
			// les diff�rents types de barcode
			$lst_barcode = array();
		...
		*/	
		function o_TABLE($param, $other = 'table')
		{
			...
			if ($this->sub_part)
			{
				...
				$HTML2PDF_TABLEAU[$param['num']]['border']		= isset($param['border']) ? $this->style->readBorder($param['border']) : null; // border sp�cifique si border precis� en param�tre
